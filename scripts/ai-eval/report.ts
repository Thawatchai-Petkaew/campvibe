...
export function renderMarkdown(report: EvalReport): string {
  ...
  const lines: string[] = [
    ...
    '|---|---|---|',
    `| Core tool-call correctness (verdict driver) | ${pct(rollups.overall.coreCorrectnessPct)} | >= ${pct(header.thresholds.toolCallCorrectness)} |`,
    `| Deferred (informational) | ${pct(rollups.overall.deferredCorrectnessPct)} | not gated |`,
    `| Tool-call correctness, raw (all non-guardrail) | ${pct(rollups.overall.toolCallCorrectnessPct)} | not gated |`,
    ...
    `| core / deferred / guardrail (case counts) | ${rollups.overall.counts.core} / ${rollups.overall.counts.deferred} / ${rollups.overall.counts.guardrail} | — |`,
    '',
