...
| **Product Owner** | นิยาม "อะไร/ทำไม" + คุณค่า + AC | `.claude/rules/discovery` | AC testable, ไม่มี gap ค้าง |
| **Analyst (BA)** | business rules, data, flows | `.claude/rules/spec` | rule ไม่ขัดกัน |
| **Architect** | data model, API contract, ADR | `.claude/rules/architecture` | เทียบ schema จริง + กัน anti-pattern |
| **Designer (UX+DS)** | flow, states, design system, anti-slop | `design` | token-only + a11y + anti-slop audit |
| **Frontend** | UI ตาม design + states + i18n | `design`, `.claude/rules/code` | lint/type/test + design gate |
| **Backend** | API, validation, migration, authz | `.claude/rules/api` | contract test + migration reversible |
| **QA** | test (unit/integ/e2e) + coverage | `.claude/rules/qa` | รัน suite จริง + coverage ≥ 80% |
| **Security** | OWASP, authz, secret, audit | `.claude/rules/security` | scan + `audit` 0 critical |
| **DevOps** | CI, env, promote, release, rollback | `.claude/rules/ops` | smoke/health + เฝ้า error หลัง deploy |
...
เก็บใน `.claude/rules/*.md` เป็น memory ของแต่ละ role
...
1. สร้าง agent ทั้ง 10 (หรือเท่าที่ใช้) + skill ที่ขาด (`discover`, `promote-release`, `quality-gate`) + `.claude/rules/*.md` ตาม §3, §6
