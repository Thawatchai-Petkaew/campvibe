...
function buildSystemPrompt(
  ...
  return [
    ...
    'When a searchCampsites or bulkAvailability call this turn returned results, open your answer with exactly ONE sentence, phrased fluently in natural Thai (never a bare field dump), explaining why this set of camps was chosen — built ONLY from that call\'s own `appliedFilters` field in the tool result, covering every dimension it actually lists: price (priceMin/priceMax), taxonomy (use ONLY each entry\'s `labelTh` Thai label, for example แม่น้ำ/ลำธาร/คลองเล็ก — NEVER its raw `code` in any form, including as a parenthetical gloss after the Thai label, for example never write "สบาย (CHIC)"; the camper never sees or needs the code, only the Thai label), location (province/near/district/subDistrict/region), keyword, and sort, when present. This sentence must contain NO parenthesis of any kind — no code, no English word, no citation — state each criterion as a plain Thai phrase alone; if you are tempted to write a word followed by "(", stop and rephrase without the parenthesis. Name ONLY what `appliedFilters` actually lists — never add, imply, or word this sentence so it sounds like a criterion the camper asked about that is absent from `appliedFilters` (the same never-claim-the-unapplied rule as the line above, extended to every dimension, not only location/terrain). Do NOT restate, acknowledge, or mirror the camper\'s own request wording in this sentence, even to make it read more naturally — a mood, vibe, or quality word the camper used (for example บรรยากาศโรแมนติก, มู้ดดี, ฟีลกู้ด, วิวสวย) must be DROPPED from this sentence entirely unless a matching filter genuinely appears in `appliedFilters`; if you mapped that mood to a best-effort filter elsewhere this turn (per the concept-map guidance), name only the FILTER you actually set and its `labelTh`, never the camper\'s own original word. For example: the camper asks for a "บรรยากาศโรแมนติก" (romantic-atmosphere) camp that also allows pets, and `appliedFilters` contains only `petFriendly:true` (no filter exists for "romantic" anywhere in this tool) — your opening sentence must name ONLY the pet-friendly criterion (for example "เลือกมาจากเงื่อนไขที่ขอไว้ คือพาสัตว์เลี้ยงไปได้") and must NEVER contain the word โรแมนติก or any paraphrase of it, even though the camper used that word — it never reached the tool as a real filter, so the reason sentence stays silent on it. When `appliedFilters` has nothing set at all (its `taxonomy` list is empty and every other field is absent), say plainly that this was a broad, general search with no specific criteria applied and invite the camper to give one — never invent a reason. Skip this opening sentence only when the turn made no searchCampsites/bulkAvailability call at all.',
