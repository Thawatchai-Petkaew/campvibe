...
.node .ncount{position:absolute;top:-6px;right:-6px;min-width:18px;height:18px;padding:0 5px;border-radius:999px;display:grid;place-items:center;font-family:var(--mono);font-size:10px;font-weight:600;color:var(--text);background:var(--glass-2);border:1px solid var(--line)}
.stage.run .node .ncount{color:var(--emerald);border-color:rgba(91,233,176,.5)}
.stage.gate .node .ncount{color:var(--amber)}
.stage.done .node .ncount{color:var(--green)}
.stage.idle .node .ncount{display:none}
...
.qrow{display:flex;align-items:center;gap:12px;padding:12px 13px;border-radius:13px;margin-bottom:9px;border:1px solid var(--line);background:var(--glass-2);text-decoration:none;color:inherit;width:100%;text-align:left;cursor:pointer;font:inherit}
...
.qrow .qm{flex:1;min-width:0;overflow:hidden}
.qrow .qm b{font-family:var(--disp);font-weight:600;font-size:13.5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:block}
.qrow .qm .tk{font-family:var(--mono);font-size:10.5px;color:var(--faint);display:block;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.kc{border-radius:12px;padding:11px;margin-bottom:9px;border:1px solid var(--line);background:var(--glass-2);text-decoration:none;color:inherit;display:block;min-width:0;width:100%;text-align:left;cursor:pointer;font:inherit}
.kc:last-child{margin-bottom:0}
.kc .kt{font-size:12.5px;color:var(--text);line-height:1.35;display:flex;gap:7px;align-items:flex-start;min-width:0}
.kc .kt svg{width:15px;height:15px;flex:none;margin-top:1px;color:var(--faint)}
.kc .kt span{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.kc .kb{display:flex;align-items:center;justify-content:space-between;margin-top:9px;min-width:0}
.kc .kr{font-size:11px;color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.kc .tk{font-family:var(--mono);font-size:10px;color:var(--faint);flex:none}
...
.gr-btn{flex:none;cursor:pointer;font-family:var(--disp);font-weight:600;font-size:13px;color:#241402;background:linear-gradient(180deg,#ffc06a,#ff9d3c);padding:10px 16px;border-radius:11px;text-decoration:none;border:none}
...
/* ---- grouping toggle (Feature | Persona) + chips ---- */
.segmented{display:inline-flex;gap:3px;background:var(--glass-2);border:1px solid var(--line);border-radius:10px;padding:3px;flex:none}
.segbtn{border:none;background:transparent;color:var(--muted);font-family:var(--disp);font-weight:500;font-size:12px;padding:5px 12px;border-radius:7px;cursor:pointer;min-height:30px}
.segbtn:hover{color:var(--text)}
.segbtn.active{background:rgba(91,233,176,.16);color:var(--emerald);box-shadow:inset 0 0 0 1px rgba(91,233,176,.32)}
.gsub{display:none}.gsub.active{display:block}
.grp{margin-bottom:18px}.grp:last-child{margin-bottom:0}
.grp-h{display:flex;align-items:baseline;justify-content:space-between;gap:10px;margin:0 2px 11px;padding-bottom:8px;border-bottom:1px solid var(--line)}
.grp-name{font-family:var(--disp);font-weight:600;font-size:13.5px;color:var(--text)}
.grp-meta{font-family:var(--mono);font-size:10.5px;color:var(--faint);white-space:nowrap}
.chip{font-family:var(--mono);font-size:9.5px;letter-spacing:.04em;padding:4px 9px;border-radius:999px;flex:none;border:1px solid var(--line);color:var(--muted);background:rgba(255,255,255,.05);white-space:nowrap;align-self:center}
.chip.persona.host{color:var(--emerald);background:rgba(91,233,176,.12);border-color:rgba(91,233,176,.34)}
...
/* ---- epic switcher modal ---- */
.swbtn{margin-left:8px;border:1px solid var(--line-2);background:var(--glass-2);color:var(--muted);font-family:var(--disp);font-weight:600;font-size:12px;padding:5px 12px;border-radius:999px;cursor:pointer;min-height:30px;display:inline-flex;align-items:center;gap:6px}
.swbtn svg{width:14px;height:14px}
.swbtn:hover{color:var(--emerald);border-color:rgba(91,233,176,.4)}
.switcher{position:fixed;inset:0;z-index:60;display:none;align-items:flex-start;justify-content:center}
.switcher.open{display:flex}
.sw-backdrop{position:absolute;inset:0;background:rgba(4,8,18,.62);backdrop-filter:blur(3px);-webkit-backdrop-filter:blur(3px)}
.sw-panel{position:relative;z-index:1;margin-top:8vh;width:min(560px,92vw);max-height:80vh;display:flex;flex-direction:column;background:var(--glass);backdrop-filter:var(--blur);-webkit-backdrop-filter:var(--blur);border:1px solid var(--line-2);border-radius:var(--r);box-shadow:0 24px 70px rgba(0,0,0,.5),inset 0 1px 0 var(--hi)}
.sw-head{display:flex;align-items:center;justify-content:space-between;padding:16px 18px 12px}
.sw-title{font-family:var(--disp);font-weight:600;font-size:16px}
.sw-x{border:none;background:transparent;color:var(--muted);font-size:15px;cursor:pointer;width:30px;height:30px;border-radius:8px}
.sw-x:hover{background:rgba(255,255,255,.08);color:var(--text)}
.sw-filter{display:flex;flex-wrap:wrap;gap:6px;padding:0 18px 13px;border-bottom:1px solid var(--line)}
.sw-fbtn{border:1px solid var(--line);background:var(--glass-2);color:var(--muted);font-family:var(--disp);font-weight:500;font-size:12px;padding:6px 13px;border-radius:999px;cursor:pointer;min-height:32px}
.sw-fbtn:hover{color:var(--text)}
.sw-fbtn.active{background:rgba(91,233,176,.16);color:var(--emerald);border-color:rgba(91,233,176,.34)}
.sw-list{overflow-y:auto;padding:11px 12px 14px;display:flex;flex-direction:column;gap:6px}
.sw-item{display:flex;align-items:center;gap:11px;padding:10px 11px;border-radius:12px;border:1px solid var(--line);background:var(--glass-2);text-decoration:none;color:inherit}
.sw-item:hover{border-color:var(--line-2);background:rgba(255,255,255,.05)}
.sw-item.current{border-color:rgba(91,233,176,.45);background:linear-gradient(160deg,rgba(91,233,176,.1),transparent)}
.sw-ic{width:34px;height:34px;border-radius:9px;flex:none;display:grid;place-items:center;background:rgba(91,233,176,.12);border:1px solid rgba(91,233,176,.3);color:var(--emerald)}
.sw-ic svg{width:18px;height:18px}
.sw-m{flex:1;min-width:0;display:flex;flex-direction:column;gap:2px}
.sw-name{font-family:var(--disp);font-weight:600;font-size:13.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.sw-meta{font-family:var(--mono);font-size:10px;color:var(--faint)}
.sw-st{font-family:var(--mono);font-size:11px;flex:none;min-width:36px;text-align:right;color:var(--muted)}
.sw-st.run{color:var(--emerald)}.sw-st.gate{color:var(--amber)}.sw-st.done{color:var(--green)}.sw-st.q{color:var(--faint)}
/* ---- ticket detail modal (CAM-286, read-only) — same glass shell family as .switcher/.sw-panel ---- */
.td-overlay{position:fixed;inset:0;z-index:70;display:flex;align-items:flex-start;justify-content:center}
.td-backdrop{position:absolute;inset:0;background:rgba(4,8,18,.62);backdrop-filter:blur(3px);-webkit-backdrop-filter:blur(3px)}
.td-panel{position:relative;z-index:1;margin-top:10vh;width:min(520px,92vw);max-height:76vh;overflow-y:auto;display:flex;flex-direction:column;background:var(--glass);backdrop-filter:var(--blur);-webkit-backdrop-filter:var(--blur);border:1px solid var(--line-2);border-radius:var(--r);box-shadow:0 24px 70px rgba(0,0,0,.5),inset 0 1px 0 var(--hi)}
.td-head{display:flex;align-items:flex-start;gap:12px;padding:16px 18px 12px}
.td-titles{flex:1;min-width:0}
.td-key{font-family:var(--mono);font-size:11px;color:var(--emerald);font-weight:700;letter-spacing:.04em;display:block;margin-bottom:3px}
.td-title{font-family:var(--disp);font-weight:600;font-size:15px;color:var(--text);line-height:1.3}
.td-close{display:inline-flex;align-items:center;justify-content:center;width:44px;height:44px;min-width:44px;border-radius:50%;background:var(--glass-2);border:1px solid var(--line);color:var(--muted);cursor:pointer;flex:none}
.td-close:hover{background:rgba(255,255,255,.08);color:var(--text)}
.td-close:focus-visible{outline:2px solid var(--emerald);outline-offset:2px}
.td-meta{font-size:11.5px;color:var(--muted);display:flex;flex-wrap:wrap;gap:6px;align-items:center;padding:0 18px 12px}
.td-meta b{color:var(--text);font-weight:600}
.td-sep{height:1px;background:var(--line);margin:0 18px 12px}
.td-body{padding:0 18px 18px}
.td-desc{font-size:12.5px;line-height:1.6;color:var(--muted);white-space:pre-wrap;word-break:break-word}
.td-desc-empty{font-size:12.5px;color:var(--faint);font-style:italic}
.td-desc-error{font-size:12.5px;color:var(--amber);display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.td-retry{font-size:11.5px;font-weight:600;color:var(--emerald);cursor:pointer;background:none;border:1px solid rgba(91,233,176,.3);border-radius:6px;padding:3px 10px}
.td-retry:hover{background:rgba(91,233,176,.1)}
.td-retry:focus-visible{outline:2px solid var(--emerald);outline-offset:2px}
.td-skel{height:13px;border-radius:4px;background:linear-gradient(90deg,rgba(255,255,255,.06) 25%,rgba(255,255,255,.12) 50%,rgba(255,255,255,.06) 75%);background-size:200% 100%;margin-bottom:8px}
@media (prefers-reduced-motion:no-preference){.td-skel{animation:hud-shimmer 1.4s linear infinite}}
@keyframes hud-shimmer{0%{background-position:200% 0}100%{background-position:-200% 0}}
.td-link{display:inline-flex;align-items:center;gap:5px;margin:0 18px 18px;font-size:11.5px;font-weight:600;color:var(--faint);text-decoration:none;padding:8px 10px;border-radius:8px;min-height:44px}
.td-link:hover{color:var(--emerald);background:rgba(91,233,176,.07)}
.td-link:focus-visible{outline:2px solid var(--emerald);outline-offset:2px}
@media (prefers-reduced-motion:no-preference){.td-panel{animation:modalIn 200ms cubic-bezier(0.23,1,0.32,1) both}}
