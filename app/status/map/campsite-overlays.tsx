...
/* ── CAM-374: shell-owned HUD chrome — relocated verbatim from SCENE_CSS ─────
   (campsite-canvas.tsx, the 2D-only stylesheet). This positioning/z-index/
   responsive show-hide CSS styles elements StatusMapShell itself renders
   (campsite-scene.tsx: .hud-topbar, .hud-left-panels, .hud-right-panels,
   .hud-map-toolbar, .hud-edge-tab, the SoundToggle button, the topbar logo)
   — NOT the 2D sprite scene. Previously it only loaded with CampsiteCanvas
   (2D), so in 3D (Canvas3D mounted instead) these controls collapsed to
   unpositioned static blocks and were painted BELOW the position:absolute
   3D canvas (z-index:5) — covering the 2D/3D renderer toggle and every
   overlay card so a user in 3D could not get back to 2D. HUD_CSS is always
   injected (both renderers), so this chrome now stays correctly positioned
   and stacked above the 3D canvas as well. Behavior-neutral relocation —
   in 2D both stylesheets still load and the rule text is unchanged. */
...
@media (max-width: 639px) {
  ...
  /* SMUX-6-fix-3 (CAM-259): same dark-green HUD glass language as .hud-signpost /
     .hud-view-toggle (this file) so the toolbar buttons read as one
  ...
  .hud-dock{bottom:60px}
  /* SMUX-6: the bottom filter row (.hud-signposts-bottom) sits above the toolbar */
