...
export default function OperatorDashboard() {
    ...
    useEffect(() => {
        fetch('/api/operator/dashboard')
            ...
            .then(responseEnvelope => {
                const d = responseEnvelope.data || responseEnvelope;
                setData({
                    stats: d.stats || { totalRevenue: 0, totalBookings: 0, campSiteCount: 0, campgroundCount: 0 },
                    bookings: d.bookings || [],
                    campSites: d.campSites || d.campgrounds || [],
    ...
    return (
        <div className="space-y-8" data-testid="page--dashboard-overview">
            {/* Chrome: renders instantly — not gated on async data */}
            <div className="flex justify-between items-end">
                ...
                {!showSkeleton && data && (
                    ...
                    >
                        <Link
                            href={canCreateCampSite ? "/dashboard/campsites/new" : "#"}
                            aria-disabled={!canCreateCampSite}
                            tabIndex={canCreateCampSite ? 0 : -1}
                        >
                            <Button
                                size="lg"
            ...
            {/* Error state — shown after loading resolves */}
            {!showSkeleton && error && (
                <div className="p-10 text-destructive bg-destructive/10 rounded-2xl border border-destructive/30">
                    <h3 className="font-bold text-lg mb-2">{t.common.retry}</h3>
                    <p>{error}</p>
            ...
            {/* Async section: skeleton while loading, real content when ready (CLS=0) */}
            <div
                aria-busy={showSkeleton}
                data-testid="section--dashboard-data"
            >
                {/* Live region — screen reader announces loading state */}
                <div role="status" aria-live="polite" className="sr-only">
                    {showSkeleton ? SR_LABEL : ""}
                </div>
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <Card className="rounded-2xl shadow-sm border-border bg-card">
                                ...
                                <CardContent>
                                    <div className="text-2xl font-bold text-foreground">
                                        {data.stats?.campSiteCount || data.stats?.campgroundCount || 0}
                                    </div>
                                    <p className="text-xs text-muted-foreground mt-1">
