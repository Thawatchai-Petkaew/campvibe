...
| `disabled` / `disabled-foreground` (CAM-662) | `0.870 0.004 214` / `0.460 0.014 214` | `0.320 0.008 220` / `0.660 0.014 214` | **disabled/inactive controls only** — flat fill (`bg-disabled`) + text/icon (`text-disabled-foreground`) for a disabled button/input/select/textarea/checkbox/filter-chip; text-only for a disabled menu/select/command item, tab, label, or the calendar's disabled day | de-emphasizing anything with **opacity** — see the rule below |
...
- **Opacity may never de-emphasize a control (CAM-662, owner ruling).** `disabled:opacity-50` (and every de-emphasis use of alpha generally) composites against whatever sits behind the element, so the resulting contrast is unmeasurable and drifts per surface. Every disabled control in `components/ui/*` uses the flat `disabled`/`disabled-foreground` pair above instead — never an opacity modifier. `npm run check:contrast` measures both tokens numerically (§6); `scripts/check-contrast.mjs` also flags remaining opacity-for-de-emphasis / alpha-on-text in report mode (non-blocking until the repo-wide backlog clears, §8).
- **Scrims/overlays are the one family of sanctioned exceptions — see-through IS the requirement, not de-emphasis.** Two call sites keep alpha on purpose, because both blend with imagery/content behind them rather than dimming a control: (1) `bg-foreground/50` or `/60` **over an image** (avatar hover scrim, gallery caption chip with `backdrop-blur`) — the correct light/dark-adaptive overlay idiom; not a CTA button, not a DS violation, do not replace with a hardcoded color; (2) the **modal/sheet/alert-dialog scrim**, which always uses `bg-overlay`, never `foreground` (CAM-661) — see the next bullet. Neither is "de-emphasis" in the CAM-662 sense (nothing here is a control signaling disabled/inactive), so neither is touched by the no-opacity-for-de-emphasis rule above.
- **A modal/sheet/alert overlay always uses `bg-overlay`, never `foreground`** (CAM-661). `--foreground` flips per theme (near-black in light, near-white in dark) — painting a backdrop with `bg-foreground/15` inverts with the theme: a correct dark veil in light mode, a **white film over the page** in dark mode. `--overlay` (`--color-overlay` in `@theme`) is a fixed dark neutral with no `.dark` twin, so it stays a dim veil in both themes. Pattern: `bg-overlay/25 supports-backdrop-filter:backdrop-blur-sm` on `DialogOverlay`/`SheetOverlay`/`AlertDialogOverlay` — `-sm`, not a larger radius: CAM-235 removed Dialog's blur entirely for open/close GPU cost, and `-sm` is the exact radius `sheet`/`alert-dialog` kept through that change with no complaint, so it is the proven-affordable ceiling.
...
| disabled | full round (unchanged) | none, `text-disabled-foreground` (flat, never opacity — CAM-662), no pointer | `--disabled-foreground` |
...
12. **CAM-662 phase 2 — opacity-for-de-emphasis + alpha, outside this story's file surface.** This story replaced every disabled-state `opacity-50` in `components/ui/*` with the flat `disabled`/`disabled-foreground` tokens (backlog 0 there). Still open, named explicitly so nobody "fixes" the wrong thing first: **~12 disabled-state `opacity-50` call sites outside `components/ui/*`** (dashboard pages, FilterModal, RegisterModal, CampgroundCard) · **alpha directly on text** (`text-foreground/70`, `text-muted-foreground/40`, …) · **hairline/tint alpha** (`border-border/60`, `bg-primary/10` — note some of these, like the icon-chip fill in §3 "Other components", are an already-approved pattern, not a defect). `scripts/check-contrast.mjs` now prints all three counts on every run in **report mode only** (never blocks — the backlog is non-zero, per the report-mode → 0 → blocking rollout rule) so the remainder stays visible until a follow-up story clears it.
