...
describe("AC-2/AC-3/AC-4/BR-3/BR-4 — expand-to-full-page toggle", () => {
  ...
  it("[unit] the Content className switches between the expanded variant and the collapsed bottom-sheet/anchored-card variant (CAM-455 reverts CAM-454's inset sliding card back to CAM-431's true-fullscreen inset-0 — see cam-455 test)", () => {
    expect(panelSrc).toContain("inset-0 duration-200 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95");
    expect(panelSrc).not.toContain("inset-4 rounded-3xl border border-border/60 lg:inset-y-4 lg:right-4 lg:left-24");
  ...
  it("[unit] BR-4: the useAiChat destructure is unchanged — expand/collapse never remounts the conversation", () => {
    ...
    // `{ onNeedsLogin }` options object (the login-gate wiring), so the
    // empty-parens call no longer exists; the invariant this test protects
    // (exactly ONE hook call) is re-asserted against the new call shape.
