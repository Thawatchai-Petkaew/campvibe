...
describe("CAM-526 (BR-2) — HORS/TENT collision: excluded here, unrelated rows untouched", () => {
  it("[normal] every AccommodationTypeEnum member is accounted for: all 5 now seeded (CAM-536 superseded the 4-of-6/2-excluded state this test originally pinned — HORS/TENT were renamed to HCMP/TSIT, see cam-536-accommodation-code-collision.test.ts; CAM-538 then dropped HCMP entirely, no Thai relevance, see cam-538-accommodation-clarity.test.ts)", () => {
    const allMembers = AccommodationTypeEnum.options;
    expect(allMembers.sort()).toEqual(["CABI", "DISP", "GROU", "RECR", "TSIT"].sort());
