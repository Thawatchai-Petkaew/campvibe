...
describe("AC-status-1/2: app/status/page.tsx must not be in the F5 diff", () => {
    it("git diff staging --name-only does not include app/status/page.tsx", () => {
        ...
        if (branch.startsWith("feature/st1") || branch.startsWith("feature/st2") || branch.startsWith("feature/st1-st2") || branch.startsWith("refactor/cut-autonomous") || branch.startsWith("feat/cam-145") || branch.startsWith("feature/cam-151") || branch.startsWith("feature/cam-156") || branch.startsWith("local/map-polish") || branch.startsWith("fix/camper-infra")) {
            return; // skip: this story (ST1/ST2/CAM-145/CAM-151/CAM-156-S6/map-polish/camper-infra) owns app/status/page.tsx
        }
        let diffOutput = "";
        try {
