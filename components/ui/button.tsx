...
// CAM-662 — disabled uses the flat --disabled/--disabled-foreground pair
// (never opacity), forced with `!` so it always wins over every variant's
// own bg/text/border below, regardless of Tailwind's internal sort order.
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-full border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/30 active:not-aria-[haspopup]:translate-y-px motion-safe:active:scale-95 disabled:pointer-events-none disabled:bg-disabled! disabled:text-disabled-foreground! disabled:border-transparent! aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        // CAM-261: rest-state tint lowered to 2%/10% (was 10%/20%) — same
        // same-hue tint/text AA fix measured for components/ui/badge.tsx
        // applies identically here (text-destructive on a bg-destructive
        // tint). hover/focus tints are unchanged (axe's static scan does not
