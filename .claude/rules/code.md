...
| "Server actions for auth + `useSession()` will sync the UI." | NextAuth v5 + App Router: a server-action `signIn`/`signOut` sets the cookie server-side but does NOT update the client `SessionProvider`, and `<SessionProvider>` with no initial `session` starts at `status="loading"` → a navbar reading the client session (or a server prop) shows stale auth until reload. Hydrate `<SessionProvider session={await auth()}>` in the root layout AND use the client `signIn`/`signOut` from `next-auth/react` for client-component auth (CAM-241/CAM-242). AND: a client-side auth/permission GATE reads the LIVE client session (`useSession()` status), never a server-rendered prop — the prop lags `router.refresh()` after a modal login, so the first press after login fails on a stale value (CAM-396). |
| "The mount effect fetches once; a plain `loadData()` in `useEffect` is fine." | React Strict Mode double-invokes the mount effect in dev, and a post-mutation refetch can overlap it — whichever response resolves LAST wins the state, so a stale earlier-issued response can silently erase data the user just created (surfaced as an e2e "flake" that was a real product race). Gate every state commit on a monotonic requestId ("is this still the latest issued call") in any async load that can run concurrently (CAM-359). |
...
| "A substring / `.contains()` match on the Thai place/keyword is fine." | Thai substring matching collides with common words — ใต้ต้นไม้→ภาคใต้, ตากแดด→จ.ตาก (DEF-1), เขาใหญ่→เขา+ใหญ่ (DEF-2). Guard any lexicon match over Thai input (boundary / `ภาค`-prefix / ambiguous-name skip-set / camping-context marker) and prove the false-match with a red regression test BEFORE the fix (CAM-501/CAM-503). |
| "The property is missing here but its siblings have it — just add it." | Chesterton's Fence runs in both directions: the ABSENCE of a property can itself be a decision. CAM-661 restored `backdrop-blur` to Dialog at `-md`; CAM-235 had removed Dialog's blur DELIBERATELY for modal open/close GPU cost, and `-md` is a larger radius than what was removed. Before adding back anything a component visibly lacks while its siblings have it, `git log -S '<the property>'` for the ticket that took it out, and restore at or below that cost (`-sm` — exactly what sheet/alert-dialog kept through CAM-235 with no complaint). Supersede the old ticket's test pin with a dated note; never silently delete a pin that is enforcing someone's reasoning (CAM-235/CAM-661). |
| "Make the field a multi-select — more flexible." | A form multi-select + zod `z.array` over a SCALAR column silently truncates to `[0]` and can write an invalid sentinel on empty — the DB kept only the first pick while the "เลือกได้หลายอย่าง" hint lied (CAM-517: campSiteType). A field's validation + form cardinality MUST match the column cardinality; match the form to storage (or make storage a relation), never bridge with a silent `[0]` (CAM-517/CAM-520). |
