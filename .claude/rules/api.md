...
| "The client sends the new `?mode=`/filter param, so the server is filtering." | A server that predates the param ignores it — zod's `safeParse` drops unknown keys by design — and returns the UNFILTERED set. The client, having correctly stopped filtering locally because trusting the server was the whole point, then reports a precise and completely wrong answer: `gates` printed "0 waiting on you, 500 cleared" while five stories waited. Client tooling and server deploy on different schedules here (`ticket-sync.mjs` reads the DEPLOYED app, never localhost), so this is the normal case, not the edge one. **The response must prove which mode/filter was applied, and a client that cannot distinguish "honoured and empty" from "ignored and everything" must refuse to interpret rather than guess** — consider rejecting an unknown `mode` with `400` instead of dropping it (CAM-595/CAM-602). |
| "It's a one-off demo/seed script — it can update the rows it finds, and `--undo` puts them back." | An undo restores what it RECORDED. CAM-663's first version overwrote real `Spot` rows having recorded nothing, and its second hardcoded `useSpotView: false` in the undo — a value that moves the camp's real capacity, since with spot-view on the capacity becomes the SUM of live pitch capacities. Four obligations, all of them load-bearing: the script is **create-only** over an env that holds real rows; the undo restores recorded prior state, never an assumed default; the pre-apply state is recorded OUTSIDE the script's own artifacts (a marker living in a row a host can delete from the UI is not a record — put it in the ticket); and every user-visible field it writes is indistinguishable from real content — naming demo pitches `CAM-663 Demo — …` put ticket ids on a camper-facing screen and destroyed the long-name case the demo existed to provide. A fake-Prisma unit suite structurally cannot catch any of this; verify the round-trip against the real target (CAM-663). |
