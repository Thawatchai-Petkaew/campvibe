...
Any story that changes HOW a value is derived, enforced, written, or SHAPED (a column becoming a relation, a hard-delete becoming a soft-delete — not just where it renders) must grep-inventory **every** reader and writer of the affected columns/fields across `app/`, `lib/`, `components/`, `scripts/`, and tests, and record them in the spec's Seams (tech.md blast-radius table or story Seams & refs) tagged **NOW / LATER / NO-CHANGE** with one line of reasoning each. Exemplar: CAM-362 tech.md §5. Shape changes count double: the S4b images CSV→relation change broke consumers one-by-one on staging (edit prefill CAM-348, list query CAM-349), and a hard→soft-delete conversion silently un-guards every aggregation/IDOR consumer that assumed deleted rows were gone (CAM-352). Lesson: CAM-355's spec enumerated 4 enforcement readers and missed the 5th (`/api/campsites/[id]/availability` — the camper date-picker), surfacing as a G3 Important and a same-PR rework round. The sweep is grep-backed, not memory-backed — name the search terms used. **The sweep must end in a SINGLE seam invariant, enforced by a test:** every consumer in the inventory shares one derivation and one semantics for edge values (`null` vs `0`, empty vs missing) — an inventory that documents divergent semantics "kept by construction" is a finding to FIX, not a note (CAM-355 preserved whole-camp `0`=unbounded while two display layers read `0`=full → users booked camps the UI called เต็มแล้ว, CAM-400). Write one invariant test that walks the matrix (edge values × modes × every reader + the write gate) and asserts they agree — per-layer tests cannot see cross-layer divergence. When a defect is found in ANY layer of a multi-consumer seam, the story's scope is the invariant across ALL layers, not the reported symptom.
...
| "The other branch's odd semantics is pre-existing — keep it unchanged-by-construction." | That is how CAM-400 shipped: CAM-355 inventoried the capacity seam, SAW the whole-camp `0`=unbounded quirk, and preserved it while `remaining-capacity` + the catalog badge read `0`=full — deterministic overbooking on camps the UI called เต็มแล้ว. Divergent edge-value semantics across a seam is a defect in scope NOW; align it and pin it with a cross-layer invariant test. |
| "Wrap the fetcher in `unstable_cache` at module level and pass the tag." | `unstable_cache` fixes `tags` at WRAP time, not per call — a module-level wrapper can never read a runtime argument, so `tags: []` (or any static list) makes every future `revalidateTag()` silently inert against that entry; a TTL masks the symptom for months. When a tag depends on a runtime argument, build the wrapper INSIDE the exported function per call (cache-hit preserved — the store keys on keyParts+args), and prove invalidation with a behavioral test: mutate → revalidateTag → assert the fetcher re-fires (CAM-353/CAM-357; inert since CAM-195). |
| "Tell the model to skip the field when it doesn't recognise the place/name." | That asks the model to decide something only the data layer knows. `searchCampsites`'s `district` description said "if you do not recognise it, leave it unset and use `keyword`" — no model recognises แม่ริม; only `AdminArea` does. It obeyed, fell back to a keyword that cannot match a district name, and returned nothing while the resolver behind it worked perfectly. Recognition belongs to a deterministic server-side pre-pass that resolves the entity and PINS the result into the turn ("you MUST set district=X"); the model's job is to use what it is handed, not to guess what exists (CAM-596). |
| "The milestone is blocked — the ADR says this feature needs those three models." | Separate the ADR's DECISION from its incidental couplings before you schedule work to satisfy it. HostOS looked blocked on `HostLead` + `Quote` + `DepositRecord` because ADR-012 §6 carried one sentence — "every manual stay carries a `leadId`" — written when leads were assumed to arrive first. That clause, not the decision, put three unbuilt models on the critical path; dropping it left the first slice needing ZERO new models against the schema already shipped. An ADR sentence that couples your work to something unbuilt is a candidate for supersede-with-a-dated-note (§16), not an automatic dependency (CAM-686/ADR-012). |
| "Adding a filterable MasterData group = seed rows + the filter modal." | A new filterable group's blast radius is the WHOLE catalog fetch pipeline (page.tsx → CatalogResults → InfiniteScrollGrid → catalog-cursor validation → `/api/campsites` GET) + BOTH AI tools (search-campsites AND bulk-availability pin their own code consts) + the host form (~6 CampgroundForm touchpoints) + the detail section — the FilterModal chip alone produces a query param the Home grid silently ignores. Enumerate all of it (CAM-515). |
