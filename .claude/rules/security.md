...
| "The fail-open try/catch bounds the risk on this public endpoint." | try/catch bounds ERRORS, not CPU — a client-supplied 8000-year date span still ran a synchronous per-night loop to completion on an unauthenticated GET (single-request event-loop hang). Any per-request loop whose iteration count is client-controlled (date range, page size, list length) gets an explicit MAX cap checked BEFORE the loop runs (CAM-344, MAX_STATUS_RANGE_NIGHTS=366). |
| "The domain is already hardcoded as the default `EMAIL_FROM`, so it's fine as the link origin too." | Those two failures are not the same size, and the existing hardcode is not precedent. An unverified SENDER domain merely fails to send — the email never leaves. A LINK origin on a domain we may not control sends the recipient — here a host, mid-booking — somewhere that is not us, from a message that stays clickable in their inbox forever, and we cannot recall it. Every outbound origin (email deep link, redirect target, webhook callback) reads from config with a real fallback (`process.env.APP_BASE_URL ?? '<a host we own>'`, already used 11× including `app/layout.tsx`), never a literal (CAM-685). |
| "Masking the userinfo (`://***@`) sanitizes the connection string for logs." | Query-string secrets (`?api_key=`, `?password=`) survive that mask into console/CI logs — the exact leak lived on in db-reset.mjs after its successor fixed it. Diagnostics that name a database print scheme+hostname ONLY (describeUrlShape pattern), never path/query/userinfo (CAM-359/CAM-369). |
