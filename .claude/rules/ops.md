...
| "`gh`/the script said merged, so it merged." | A chained `&& echo merged` can lie. Confirm with `gh pr view --json state`; a branch BEHIND after a concurrent merge makes the required status check "expected" and blocks even `--admin` → update the branch (merge base in) → re-run CI → merge (CAM-203). NEVER delete a branch or mark the story Done in the SAME batch as the merge — CAM-504 did, on a merge that was actually blocked-behind, recoverable only because the commit was still in the object store; gate ALL cleanup on a confirmed `state=MERGED`. **Recurred at CAM-680** — the orchestrator had been citing this very row all session, then batched `--state Done` with the merge command anyway: PR #760 went BEHIND the instant #759 landed, the merge was refused with "2 of 2 required status checks are expected", and the board read Done over code that was not on `dev`. Knowing the rule does not enforce it. Merge → `gh pr view --json state` → close are THREE separate tool calls, never one batch. |
| "`npm install` / `npm update` inside the agent's worktree only touches that worktree." | `scripts/worktree-setup.sh` symlinks node_modules for speed and isolates ONLY `.prisma`/`@prisma` (CAM-579) — every other package is a symlink INTO the main tree, so an install/update writes THROUGH it and mutates the owner's live dev tree. A CAM-687 `npm update` left ~150 real packages empty in the owner's running dev server mid-session. Dependency work runs where `node_modules` is real (its own `npm ci`), never in a symlink-shared worktree; and a tree-wide repair is verified package-by-package (`find -newermt` + spot-read actual files), never inferred from a clean `git status` — the lockfile can be pristine while the packages are hollow (CAM-687). |
| "The staging URL returns 200, so the new build is live." | The OLD build also returns 200 — an availability check cannot date a deploy. Probe something that exists ONLY in the new build (a route that used to 404, a testid the change introduces) and assert the DISCRIMINATING value: `/api/notifications` answering 401 instead of 404 proves the deploy; 200 on `/` proves nothing. Equally, **never confirm a deployed change by grepping served HTML for human-readable words** — a curl for the CAM-666 price headline matched host-authored `feeInfo` prose containing the same words, and was reported to the owner as proof before the mistake was caught. Structural anchors only (CAM-186, CAM-666/672). |
| "The sync copies every model, so the data is complete." | A copy list naming only models silently drops implicit Prisma m2m join tables — `db:sync-from-staging` omitted `_CampSiteToMasterData`, so synced camps had 0 options and Home filters returned 0 in prod. Include implicit join tables (`_A_to_B`, A=owning id / B=related), truncate+copy them in FK order after the models, and verify by row-count after sync (CAM-497). |
...
| "`e2e-regression` is advisory (non-required), so a red one doesn't block the merge." | Non-required means the branch-protection ROBOT won't block you; it does not mean the signal is noise. On this repo only `quality-gate` + `ai-guardrail-gate` are required, and e2e is the ONLY check that exercises the real integration a unit suite mocks away — so a red e2e on a chat/booking-surface PR is a defect report until root-caused, exactly like a flaky test (qa.md). CAM-700 merged on advisory-red e2e and shipped a real detail-parse regression to `dev` (two stale mocks, fixed forward as CAM-707) before it was caught. The rule: on any PR touching an e2e-covered surface, treat e2e red as blocking-in-practice — root-cause it, then either fix the code or update the test IN THE SAME PR (the reader/writer rule: a behaviour change owns the tests it breaks), before merge (CAM-700/CAM-702). |
...
| "Committed the fix — now check CI." | CI runs the REMOTE branch, not your local commit. A fix committed locally but NOT pushed makes the gate run the stale remote branch and fail confusingly while the local run is green (CAM-519: a guardrail-gate fail chased for minutes was an un-pushed fix). Always `git push` BEFORE polling CI / re-reading a gate result. |
