...
6. **Workflow / Process** — numbered steps. Be specific: `Run \`npm run lint\` · \`npm run typecheck\` · \`npm test\``, not "verify the build". Reference real artifacts (`prisma/schema.prisma`, `ai-planning/templates/STORY-TICKET.md`, `node scripts/linear-sync.mjs audit`). Keep the stack, 3-env flow, and gates G1–G5 intact.
