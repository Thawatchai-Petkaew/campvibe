...
Read first: `docs/specs/README.md` (the SoT split + structure) · `.claude/SKILL-AUTHORING.md` (doc style) · the templates in `.claude/templates/*`.

## Quick Reference

```text
intake (new story)   → node scripts/ticket-sync.mjs scaffold <CAM-id>     # create folder + story.md skeleton only (role artifacts on-demand)
each role acts        → author YOUR artifact from `.claude/templates/*` when you act + keep its `status:` header = ticket state
every gate            → node scripts/ticket-sync.mjs index                # regen docs/specs/INDEX.md
...
Layout: `docs/specs/<feature-slug>/feature.md` · `<epic-slug>/epic.md` · `<CAM-id>-<story-slug>/{story,design,tech?,test,review,release}.md`. feature = the ticket's `featureName` · epic = the ticket's `epicId` FK · persona = the ticket's `persona` column (header, not a folder).
...
`docs/specs/README.md` · `.claude/templates/*` (the blanks) · the story's ticket (`CAM-id`, with `featureName` = feature and `epicId` = the epic ticket) · `APP_BASE_URL` + `STATUS_TOKEN` in `.env` for `scaffold`/`index`/`audit` (`scripts/ticket-sync.mjs` is a thin HTTP client over `/api/tickets/*`). Know the role→artifact map (Standards below).

## Workflow

1. **Scaffold at intake.** `node scripts/ticket-sync.mjs scaffold <CAM-id>` → creates the feature/epic containers (idempotent) + the story folder with **`story.md` only**, seeded from the **`.claude/templates/story.md` skeleton** (headers prefilled from the ticket — feature/epic/persona/status). Files are the content SoT for the story, so scaffold no longer copies `ticket.description` in — the PO fills the skeleton directly; the ticket's own `description` stays status/summary + a pointer back to this file. Role artifacts (`design/tech/test/review/release.md`) are **NOT** pre-created — each role authors its own from `.claude/templates/*` when it acts. Re-running never clobbers existing content. Then the **PO FILLS `feature.md` + `epic.md`** — scaffold only *stubs* them with `<placeholder>`: `feature.md` = overview + architecture/design overview + the epic→story rollup; `epic.md` = why/scope + the story rollup. `feature.md`/`epic.md`/`story.md` are **PO-authored + required at intake/G1** (and the `epic.md` rollup is kept current as stories move) — *not* on-demand like the role artifacts.
...
- ✅ `scaffold CAM-118` → `docs/specs/wishlist/wishlist-core/CAM-118-toggle-heart/` with 6 headed files; PO fills `story.md` (`AC-1`/`AC-2`, `BR-1`), qa fills `test.md` (`AC-1 → __tests__/…`).
...
- `docs/specs/README.md` — the store contract · `.claude/templates/*` — the blanks · `scripts/ticket-sync.mjs` — `scaffold`/`index`/`audit`.
...
- **Ownership (who writes what):** PO/analyst → `feature.md`/`epic.md`/`story.md`; architect → `feature.md ## Architecture` + `story.md ## Data` + `tech.md`(optional) + ADRs; designer → `design.md` (only when the story has UI); qa → `test.md`; security → `review.md`; devops → `release.md`; orchestrator → scaffold + `INDEX.md` + audit.
...
- [ ] `node scripts/ticket-sync.mjs index` regenerated `docs/specs/INDEX.md`; `… audit` is clean (every `roleHistory` entry has its artifact present; a no-UI story has no `design.md` and that is fine).
