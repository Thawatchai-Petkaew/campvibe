...
5. Record the PR # + Vercel preview URL into the story's `release.md` under `## PR & preview` (durable ship record — see the `delivery-artifacts` skill).
...
- `delivery-artifacts` skill — the story's `release.md` where the PR # + preview URL are recorded (`## PR & preview`).
