...
- [ ] **Delivery artifact authored** — when the story has UI, author `design.md` from the template under `docs/delivery/<feature>/<epic>/<CAM-id>-<story>/` (from `.claude/templates/*`), with its `status:` header kept = the Linear state.
...
- **Delivery artifact** — when the story has UI, author `design.md` from the template under `docs/delivery/<feature>/<epic>/<CAM-id>-<story>/` (from `.claude/templates/*`), keeping its `status:` header = the Linear state (files = content SoT, Linear = status SoT).
