...
| CAM-303 cross-source guard | spec=opus (ground-truth) · build=sonnet(qa) · orch=fable | spec ~9 min · build ~12 min | opus 99k · sonnet 192k · total ~291k | ✓ all checks | 0 | 0 | **4 proactive** (core already-satisfied verdict — refused to fabricate work · found the REAL gap: unguarded 3-source interaction · surfaced catalog-holds product gap (CAM-344) + dead paths (CAM-345) · Prove-It via mutation) | **1 (Auto mode: G1 folded into G3, orchestrator-approved)** | 0 (window open) | first spec-lite story; ground-truth-first prevented ~300 lines of fabricated "service consolidation" work |
