...
> **แก้ไข 2026-07-28 ([ADR-016](../adr/ADR-016-camper-direct-booking-and-in-chat-completion.md)):** camper **จองตรงได้** — `BookingReadinessGate` ไม่ได้ gate ปุ่มจองฝั่ง camper อีกต่อไป (guardrail 4 ยกเลิก, §4) สิ่งที่ยังเลื่อนไป M7.5 คือ **payment** ไม่ใช่ booking

## 1. Launch funnel

CampVibe เปลี่ยนจาก marketplace-จองสาธารณะ**พร้อมชำระเงินออนไลน์** ไปเป็น **HostOS-first**: **payment สาธารณะ** ถูกเลื่อนออกจาก initial launch จนกว่าจะผ่าน BookingReadinessGate (§3, M7.5)

> **การจองฝั่ง camper ไม่ได้ถูกเลื่อน** (ADR-016, 2026-07-28) — ปุ่ม `จอง` → `POST /api/bookings` → Booking สถานะ `PENDING` ทำงานอยู่จริงตั้งแต่ M0 และยังทำงานต่อไป สิ่งที่ไม่มีคือการชำระเงิน (ไม่มี payment gateway ในโค้ด) ไม่ใช่ตัวการจอง
...
funnel ด้านบนคือเส้นทางฝั่ง **host** (HostOS) · เส้นทางฝั่ง **camper** ที่ทำงานอยู่จริงวันนี้คือ `Discover → จอง → Booking PENDING` (ADR-016) — สองเส้นนี้ยังไม่บรรจบกัน ดู RISK-2 ใน ADR-016
...
| **AI Assistant** | Discover→**Book** — ค้นหา/ถามสด แล้วพาไปจนจบการจอง **ในแชท** (รอบ 1: guided flow วัน→จำนวนคน→สรุป แล้วส่งต่อไปหน้าจองที่กรอกไว้ให้แล้ว, แชทไม่เขียนข้อมูลเอง — ADR-016) | M2 | tool layer + chat ใช้งานได้บน staging; in-chat booking = epic CAM-630 |
...
| **M2 — AI Discover→Book** | NL search, live availability Q&A, การ์ดในแชท, **พาไปจนจบการจอง** (รอบ 1: guided flow → หน้าจองที่ prefill แล้ว แชทไม่เขียนเอง — ADR-016); runtime = OpenRouter | เข้า: M1 (data ถูกต้อง) · **ไม่ต้องรอ lead inbox อีกต่อไป** — ปลายทางคือ `POST /api/bookings` ที่มีอยู่แล้ว · ออก: camper จบการจองจาก guided flow ได้จริงบน staging | 6–10 สัปดาห์ |
...
| **M7.5 — OPTIONAL Payment/Checkout** | เปิด **การชำระเงินออนไลน์** (payment/checkout) — **readiness-gated ต่อ campsite** ไม่ใช่ทั้งแพลตฟอร์มพร้อมกันครั้งเดียว · **หมายเหตุ (ADR-016):** ตัว *การจอง* ฝั่ง camper ไม่ได้รออยู่ที่นี่ — เปิดใช้อยู่แล้ววันนี้ ที่ยังไม่มีคือเงิน | ผ่าน **BookingReadinessGate** ทั้ง 5 ข้อ (เจ้าของตัดสินใจเป็นราย campsite): (1) HostOS daily-active adoption ≥ เป้า · (2) map/zone/pitch completeness ≥ เป้า · (3) double-hold incidents = 0 · (4) cancellation/refund policy ครบ · (5) support playbook + audit trail พร้อม | readiness-gated, ไม่ตั้งเวลาตายตัว |
| **M8 — Secondhand Marketplace** | ซื้อขายอุปกรณ์มือสอง (ลำดับสุดท้ายตาม blueprint) | เข้า: M7 เสร็จ · ออก: TBD | not measured |

~~จนกว่า campsite หนึ่งจะผ่าน gate: ทุก CTA ที่ดูเหมือน "จอง" ต้องใช้คำ **สอบถาม / ขอราคา / ส่งคำขอ** แทน~~ — **ยกเลิกแล้ว 2026-07-28 ([ADR-016](../adr/ADR-016-camper-direct-booking-and-in-chat-completion.md))** กฎนี้ไม่เคยถูกบังคับใช้ในโค้ดเลย และ camper จองตรงได้อยู่แล้วตั้งแต่ M0 · ปุ่มฝั่ง camper ใช้คำว่า `จอง` ตามเดิม (ดู §4 ข้อ 4)
...
2. **โมเดลไม่เขียนเงิน/availability** *(แก้ไข 2026-07-28 — [ADR-016](../adr/ADR-016-camper-direct-booking-and-in-chat-completion.md): แคบลง ไม่ได้ยกเลิก)* — โมเดลไม่ commit การเปลี่ยนแปลงที่กระทบเงินหรือที่ว่างด้วยตัวเอง **การเขียนจริงเป็นโค้ด deterministic เสมอ และเกิดจาก "การยืนยันครั้งสุดท้ายของ camper เอง"** (`confirm จริง = โค้ด ไม่ใช่โมเดล`)
   **สิ่งที่เปลี่ยนไป พูดตรง ๆ:** การจองของ camper **ไม่ต้องรอ host ยืนยันอีกต่อไป** — ของเดิมมัดสองเรื่องไว้ด้วยกัน (ก) โมเดลห้ามเขียน = **คงไว้ และเป็นครึ่งที่สำคัญ** (ข) ต้องมีมนุษย์ฝั่ง host อนุมัติก่อน = **ตัดออกสำหรับเส้นทางการจองของ camper** · ฝั่ง HostOS (ข้อ 1 และ 3) ไม่เปลี่ยน
...
4. ~~**กฎคำ CTA** — ตราบใดที่ campsite นั้นยังไม่ผ่าน BookingReadinessGate ปุ่ม/ข้อความที่ดูเหมือน "จอง" ต้องเปลี่ยนเป็น สอบถาม/ขอราคา/ส่งคำขอ เสมอ~~
   **ยกเลิกแล้ว 2026-07-28 ([ADR-016](../adr/ADR-016-camper-direct-booking-and-in-chat-completion.md))** — บันทึกไว้ให้ครบว่า **กฎนี้ไม่เคยถูกบังคับใช้ในโค้ดเลย** ตั้งแต่วันที่เขียน (ไม่มีธง `BookingReadinessGate`/`canBook`/`bookingEnabled` ใด ๆ ใน `*.ts|*.tsx|*.prisma` — ตรวจ 2026-07-28) ขณะที่ camper จองตรงได้จริงมาตลอด การยกเลิกนี้จึงเป็นการ**รับรองพฤติกรรมที่ ship อยู่แล้ว ไม่ใช่การเปลี่ยนพฤติกรรม** · เลข 4 ยังอยู่เพื่อเป็นบันทึกการยกเลิก **ไม่ใช่กฎที่ยังบังคับใช้** — อย่าอ่านย้อนกลับเป็นกฎ
...
[ADR-011](../adr/ADR-011-strategy-pivot-hostos-first.md) (ตัดสินใจ + rationale + alternatives — **superseded in part** โดย ADR-016) · [ADR-016](../adr/ADR-016-camper-direct-booking-and-in-chat-completion.md) (camper จองตรง + assistant ปิดการจองในแชท; guardrail 4 ยกเลิก, guardrail 2 แคบลง) · [product-plan.md](product-plan.md) (feature backlog รายละเอียด — ต้อง re-sequence ตาม ladder นี้ในรอบถัดไป) · [ai-product-roadmap.md](ai-product-roadmap.md) (AI requirement table — cluster C ต้อง amend ตาม ADR-009 `## Amendment 2026-07`) · [business.md](business.md) (revenue model ยังเป็น TODO — ทิศทางที่เสนอบันทึกไว้ใน ADR-011 เท่านั้น รอเจ้าของยืนยัน)
