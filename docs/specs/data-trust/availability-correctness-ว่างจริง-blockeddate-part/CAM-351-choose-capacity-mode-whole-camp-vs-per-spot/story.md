...
Scope: the **capacity-mode choice** in the host camp form (create + edit) + the whole-camp entry path + a read-only per-spot derived-total display + safe mode-switching that never deletes data + the listing-completeness "zones" fairness fix. Reuses the existing `useSpotView` flag and `Spot.maxCampers` — **no schema change, no migration**. Does NOT build the spot-management screen (that is **CAM-352**) and does NOT change any availability/booking math (a named follow-up).
Depends on: **CAM-352** for full PER-SPOT usability (a host cannot manage spots in-app until CAM-352 ships the screen — see the sequencing note below). Coordinates with CAM-304 completeness rule + CAM-343 holds UI (already shipped; not a blocker).
Sequencing: build **CAM-352 (spot-management screen) first or in the same wave** — PER-SPOT mode is only usable once hosts can create/edit spots in-app. CAM-351 alone still ships value for WHOLE-CAMP camps (the owner's primary case) and reframes the mode choice, so it can merge independently, but PER-SPOT is a dead end for a brand-new camp without CAM-352.
...
- BR-8 **Entry point to spot management:** in PER-SPOT mode the Capacity card links to the spot-management screen built by **CAM-352** (`/dashboard/campsites/{id}/spots`). That route returns 404 today; CAM-351's own "Create Spots" CTA must not ship a dead link — if CAM-352 lands first (recommended), link to it; if CAM-351 ships alone, show the honest derived-total display + empty state (AC-3/AC-4) and gate the CTA on `isEditing`, not a 404 link. (proves AC-3, AC-4; depends on CAM-352)
