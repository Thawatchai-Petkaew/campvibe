...
- Story-specific: Prove-It, measured honestly (`.claude/rules/performance.md` metric-honesty). At this repo's default OS/browser font scale, headless Chromium at 320/390px renders the worst pair (`ลานกางเต็นท์`/`แคมป์ด้วยรถ`) on the PRE-fix source ~15px short of touching, not overlapping — a real but narrow near-miss, not a repro, in that exact harness. Scaling the root font-size to 24px (~150%, a realistic phone "Larger text" setting many devices ship at — `min-w-[56px]` is a literal px value that does not scale with the label's rem-based `type-caption` the way real device text-scale would) reproduces the EXACT named overlap on the pre-fix source and the fix removes it; that scaled case is the spec's real red-before/green-after Prove-It. Confirmed manually before commit: reverting `shrink-0` alone (testids kept) turned exactly that one test red with `"campground" overlaps "carCamping" at 150% text scale`, all others stayed green; restoring `shrink-0` turned it green again.
- Gate = `/quality-gate` (lint 0 errors · typecheck clean · full vitest suite green except the known pre-existing `__tests__/delivery-client.test.ts` env-dependent failure · `check:ds`/`check:palette` PASS — `check:ds` folds in the M1/M2/M3 scale guard, there is no separate `check:scale` npm script).

## Changelog
- v1 (2026-07-26) — created
- v2 (2026-07-26) — Self-verify updated with the measured e2e results after building the fix: the plain 320/390px checks pass with the pre-fix source too close to call reliably (a real but narrow, non-negative margin), so a font-scale Prove-It case was added as the genuine red-before/green-after regression proof; noted honestly rather than claimed as a clean repro at default scale.
