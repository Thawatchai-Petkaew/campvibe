...
- BR-4 Seven type roles, four of which step at 768px: `type-display` 30→36 · `type-heading-1` 24→30 · `type-heading-2` 20→24 · `type-heading-3` 16→18. Three do NOT step, on purpose: `type-body` 16px (below 16px iOS Safari auto-zooms a focused input, and 16px is the body-copy floor), `type-label` 14px, `type-caption` 12px. (proves AC-1)
