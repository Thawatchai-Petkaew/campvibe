...
- Story-specific: `lib/campsite-filters.ts` untouched (not in the diff) · `Location.province`'s stored value/derivation unchanged (same `language === 'th' ? ... : ...` expression, same `ThailandLocation` source, just relocated into the component) · rebased onto `dev` (picked up CAM-548/549/550/552); CAM-548's `CampgroundDetailClient.tsx` reuses `buildLocationText`, which reads only `province`/`provinceTh`/`district` — unaffected, `cam-548-detail-location.test.ts` re-run green (15/15) · full suite re-run as the last act (grepped `__tests__/` and `e2e/` for `LocationPicker`/`province`/`district` — two suites pinned the OLD flat single-search UI's exact copy/DOM shape: `__tests__/menu-hover-contrast.test.ts` AC-loc-subtitle and `e2e/regression/ac5-create-camp.spec.ts`; both were flagged first, then fixed — updated to describe the new UI, never weakened/deleted — after the coordinator expanded the file surface to exactly those two files)
- Gate = `/quality-gate` · Done = every AC verified on localhost (dev DB) before merge into `dev`

## Changelog
- v1 (2026-07-26) — created
- v2 (2026-07-26) — file surface expanded to the two flagged sibling suites; both fixed instead of left flagged. Rebased onto `dev`. Added the edit-prefill-gap finding, the `Location.province` byte-identical confirmation method, and a CAM-562 forward-looking note (canonical storage format for a future reverse-geocode backfill) to the PR body / tech.md.
