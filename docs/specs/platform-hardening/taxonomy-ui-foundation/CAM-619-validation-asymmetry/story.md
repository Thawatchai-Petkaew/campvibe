...
| AC-2 | A host submits `priceLow: 5000, priceHigh: 1000` (low > high) on **create**, or a negative price / a price over 100,000 on create or update | `POST` (order+range) or `PUT` (range only — see BR-2 v2 correction) is called | `400` with `ราคาต่ำสุดไม่สามารถมากกว่าราคาสูงสุดได้` (order, create only) or `ราคาต้องอยู่ระหว่าง 0–100,000 บาท` (range, both) | No write happens | EC-2 |
...
- BR-2 `priceLow`/`priceHigh` are `.finite().min(0).max(100000)` (matches `extraFeeAmount` in the same file + the `pricePerNight` catalog row, `.claude/rules/ux.md` §2) on **both** create and update — unchanged by the v2 correction below. The `priceLow<=priceHigh` ordering is enforced by a shared `isPriceOrderValid()` helper, called from `POST /api/campsites` (create) only — never a top-level zod `.refine()`/`.superRefine()` on `campSiteSchema` itself, because `.partial()` is called on that exact export by both the PUT route and the client-side pre-check in `components/CampgroundForm.tsx` (outside this story's file surface); a `.refine()` wrapper turns the schema into a `ZodEffects` with no `.partial()` method and would break both call sites at compile time. (proves AC-2)
  - **v2 correction (CI regression, PR 700):** the order check was ORIGINALLY also called from `PUT /api/campsites/[id]`, with a post-save projection reading the STORED value for whichever side a partial request omitted. CI caught `e2e/regression/ac1-edit-round-trip.spec.ts` failing (400 instead of 200): `CampgroundForm.tsx` sends the FULL current form state on every save (both price fields, not a field-level diff), so a host editing only `priceLow` — leaving `priceHigh` at its previously-saved value — is an ordinary single-field edit, not a malformed request. Reproduced directly against the route handler + the real seeded camp (`khao-yai-camping-site-2`, priceLow 250→777, priceHigh unchanged at 600): a real `400 ราคาต่ำสุดไม่สามารถมากกว่าราคาสูงสุดได้`. The order check is now removed from PUT entirely — a deliberate, stated asymmetry (not a gap): a brand-new listing being created has no prior state a host could be mid-way through reconciling, so enforcing order at create time is safe; an existing listing's host may legitimately update one price bound now and the other later. Pinned by `__tests__/cam-619-campsites-rate-limit.test.ts`'s "PUT does NOT enforce priceLow<=priceHigh" block.
...
- EC-2 IF `priceLow > priceHigh` is sent on **create** THEN the request is rejected `400` — but IF a partial `PUT` sends only one side (leaving the other at its previously-saved value, even if that produces an inverted pair) THEN it is ACCEPTED (v2 correction — BR-2).
...
- v2 (2026-07-28) — CI caught `e2e/regression/ac1-edit-round-trip.spec.ts` regressing (a normal single-field host price edit 400'd). Root cause: BR-2's `priceLow<=priceHigh` order check, originally enforced on both create and update, ran on PUT against `CampgroundForm.tsx`'s full-form-state submission and blocked an ordinary edit. Fix: the order check now runs on `POST` (create) only; the `.min(0).max(100000)` range bound is unchanged on both. AC-2/BR-2/EC-2 updated; new regression tests added pinning PUT's corrected behavior.
