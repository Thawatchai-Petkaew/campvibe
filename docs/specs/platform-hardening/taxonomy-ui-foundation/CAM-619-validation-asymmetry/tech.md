...
- Ordering (`priceLow<=priceHigh`) is enforced by `isPriceOrderValid()`, exported from `lib/validations/campsite.ts`, called from BOTH `POST`/`PUT` route handlers AFTER a successful `campSiteSchema(.partial()).safeParse()` — deliberately NOT a top-level `.refine()`/`.superRefine()` on `campSiteSchema` itself. `campSiteSchema.partial()` is called by `app/api/campsites/[id]/route.ts` (in this story's surface) AND `components/CampgroundForm.tsx`'s client-side pre-check (`campSiteSchema.partial().safeParse(campPayload)`, line ~714 — OUTSIDE this story's allowed file surface). Wrapping the object in `.refine()` turns the exported const into `ZodEffects<ZodObject<...>>`, which has NO `.partial()` method — that would break BOTH call sites at compile time. `isPriceOrderValid` is a plain function instead.
- `PUT` computes the POST-SAVE PROJECTED value for whichever side the request omits (`data.priceLow !== undefined ? data.priceLow : existing.priceLow == null ? null : Number(existing.priceLow)`, mirroring the `isPublishTransition` projection pattern a few lines below in the same handler) — so a partial update that sends only `priceHigh: 100` against an already-stored `priceLow: 200` is still caught, not just the two fields literally present in this one request body. `== null` (not `!== null`) treats an absent/`undefined` stored value the same as an explicit `null` — defends the check against a test double that never set the field, distinct from a real inverted price.
...
- **v2/v3 history (CI regression, PR 700, then owner override):** `PUT` was briefly changed to NOT call `isPriceOrderValid` after CI's `e2e/regression/ac1-edit-round-trip.spec.ts` failed (`putRes.status()` was `400`, not `200`). Reproduced directly against the real seeded camp `khao-yai-camping-site-2` (`priceLow: 250, priceHigh: 600`) by calling the actual route handler with the exact body `CampgroundForm.tsx` sends for that edit (`{ priceLow: 777, priceHigh: 600 }` — the form submits the FULL current form state on every save, not a per-field diff, so `priceHigh` is always present, unchanged, in the body): confirmed real `400 { "error": "ราคาต่ำสุดไม่สามารถมากกว่าราคาสูงสุดได้" }`. The initial hypothesis (comparison running against an `undefined` omitted field) was checked and ruled out — both fields were genuinely present and genuinely inverted relative to each other.
  - **v2 (initial fix, later overridden):** removed `isPriceOrderValid` from `PUT` entirely, kept on `POST` only, reasoning that create-vs-update was a legitimate, statable asymmetry.
  - **v3 (owner override, final):** the 400 was CORRECT — `priceLow: 777 > priceHigh: 600` is a genuinely inverted price band; if persisted, the card renders it as e.g. `฿777–600`, which is literally the defect this story exists to close. The e2e FIXTURE was the actual defect: its `777` value was picked arbitrarily and happened to exceed the seed's `priceHigh`, but the spec's own name/intent ("updated nameTh + priceLow persist across save + reopen") only requires an in-band value to prove the round-trip. **Fix:** restored `isPriceOrderValid` on `PUT` (identical to `POST`, no create-vs-update asymmetry); corrected the e2e fixture (`e2e/regression/ac1-edit-round-trip.spec.ts`) from `777` to `450` (in-band, still distinct from the seed's `250` so the round-trip is real, not a no-op).
  - **Recorded follow-up (not built in this story):** the underlying UX gap is real — a host who edits only `priceLow` via a form that always submits both price fields can get a `400` naming `priceHigh`, a field they never touched. The fix belongs client-side: `CampgroundForm.tsx` should run the same `priceLow<=priceHigh` check before submitting (mirroring its existing inline `minPriceError`/`maxPriceError` display, which today only shows a warning — it does not block submit) so the host sees the conflict named against the field they'd expect, before a round trip to the server. Out of this story's file surface (`components/**`); track as a new ticket if prioritized.
...
- v2 (2026-07-28) — initial fix: `isPriceOrderValid` scoped to `POST` (create) only after CI's `e2e/regression/ac1-edit-round-trip.spec.ts` caught a regression.
- v3 (2026-07-28) — v2 overridden: restored `isPriceOrderValid` on `PUT` (both routes enforce order identically again); the e2e fixture (not the guard) was the actual defect and was corrected instead (`777` → `450`, in-band). See §2's v2/v3 history paragraph for the full repro + reasoning + the recorded client-side follow-up.
