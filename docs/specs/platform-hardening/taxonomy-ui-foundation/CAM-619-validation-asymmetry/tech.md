...
- `PUT` computes the POST-SAVE PROJECTED value for whichever side the request omits (`data.priceLow !== undefined ? data.priceLow : existing.priceLow != null ? Number(existing.priceLow) : null`, mirroring the existing `isPublishTransition` projection pattern a few lines below in the same handler) — so a partial update that sends only `priceHigh: 100` against an already-stored `priceLow: 200` is still caught, not just the two fields literally present in this one request body.
