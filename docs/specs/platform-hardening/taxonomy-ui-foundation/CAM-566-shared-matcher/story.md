...
| AC-4 | `scripts/backfill-cam-562-subdistrict-geocode.mjs` (PR #646) imports `matchAdminAreaByName` directly from `scripts/backfill-cam-563-location-admin-area.mjs` | This story's consolidation merges to `dev` | That import's module path AND 4-arg call signature are unchanged | PR #646 needs no rebase for this import (stated explicitly in this PR's body, per the coordinator's request) | EC-4 |
...
- EC-4 IF PR #646 rebases onto `dev` after this story merges THEN its `import { matchAdminAreaByName } from './backfill-cam-563-location-admin-area.mjs'` resolves unchanged — same path, same signature, same `.id`-bearing return shape it already reads (BR-4).
...
## Seams & refs
- Reuse: `lib/geo/admin-area-match.ts` (new) is the ONE owner of `normalizeAdminName`/`matchAdminArea`. `app/api/geocode/_shared.ts`, `app/api/location/route.ts`, and `scripts/backfill-cam-563-location-admin-area.mjs` all import it — the last one via a re-export that preserves its old export names/signature for its own live consumer (`scripts/backfill-cam-562-subdistrict-geocode.mjs`, PR #646). Full inventory of every matcher-adjacent location (why each is/isn't touched) is in `tech.md`.
...
## Self-verify
- AC-1..5 → unit (`__tests__/cam-566-admin-area-match.test.ts`, new) + integration (existing `cam-554-geocode-routes.test.ts` / `cam-563-location-admin-area-backfill.test.ts` / `cam-563-location-route-admin-area.test.ts`, run **unedited**).
- Story-specific: no migration; `scripts/backfill-cam-562-*` never touched (confirmed by the PR's file list); full suite run as the LAST act before handoff.
