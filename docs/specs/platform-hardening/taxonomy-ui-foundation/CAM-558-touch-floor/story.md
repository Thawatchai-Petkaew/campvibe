...
- v3 (2026-07-26) — CI (`e2e-regression` job, PR #645) failed AC-1 with "Received: 0" on the arrow-button locator — the arrows were never in the DOM, not merely under 44px. Root-caused by reproducing the CI environment locally (`CI=true PW_REGRESSION=1 npx playwright test --project=regression`, ALL regression specs together against one fresh seeded Postgres, matching the CI job exactly): confirmed, not assumed, that the cause is `CatalogResults.tsx`'s cached path — `page.goto("/")` with no filter sets `useCache=true`, serving `getDefaultCatalog()` (an `unstable_cache` read tagged `CATALOG_TAG`, `lib/catalog-cache.ts`). Isolated, this story's own test is the FIRST request against a fresh dev server, so the cache is empty and the PUT-then-navigate sees fresh data. When CAM-549/540/550/560/561's specs (which all share one dev server + one seeded DB in CI) navigate to `/` FIRST, they warm that cache with the camp's ORIGINAL single image; this story's `giveCampTwoImages()` PUT does call `revalidateTag(CATALOG_TAG)`, but by the time the earlier-run specs already primed the entry, the DOM the test asserts against can still be the pre-revalidation snapshot for that render pass — captured directly in a Playwright error-context page snapshot showing the card with exactly one `<img>` and no arrows, immediately after a successful PUT. Fix: `gotoCampSearch()` now navigates via `?keyword=ภูกระดึง` (a Thai substring unique to this camp's `nameTh`), which sets `isSearchActive=true` and routes to the LIVE (uncached) Prisma query every time — deterministic regardless of what earlier specs warmed or CI's file execution order. Also added explicit precondition assertions (`toHaveCount(1)` on the card locator, then on the arrow buttons, each with a message naming what unmet precondition would produce a false "0 elements" read) so a future regression here fails with a clear diagnosis instead of a bare count-0. Audited the other 6 assertions in the file (AC-2/3/4/5) for the same class of fragility: all of them measure the `Navbar`, which is static shell rendered outside `CatalogResults`' Suspense/cache boundary — none depend on catalog cache state, confirmed not fragile. Verified the fix with 3 full local reproductions of the CI job (fresh seeded DB each time, `CI=true`, all 38 regression spec files together, `workers: 1`): this story's 7 tests passed in all 3; the environment's own known pre-existing flake (`ac3-logo-roundtrip.spec.ts`, documented in `e2e/regression/README.md` "Known defect (AC-3)") failed once, unrelated to this story.
