...
| AC-4b | At the `date` step, empty slots | A date CHIP is tapped (submitted as `{kind:"chip", slots:{checkIn,checkOut}}`) | N/A (no UI in this story) | Advances with that EXACT slot value directly, with NO round-trip through `resolveDatesCore`/`parse` at all (the chip already knows its own value) | EC-2b |
...
- BR-4 A chip is a UI affordance that already knows its own slot value — `BookingFlowInput` has `{kind:'chip', slots}` alongside `{kind:'text', text}` and `{kind:'cancel'}`, and a chip NEVER round-trips through a step's `parse` (v1 modeled a chip as pre-formed text through the SAME parse path as typing; a G3 review reproduced that a bare ISO date submitted as text does not parse via `resolveDatesCore`, so v2 gives the chip its own bypass instead). Typed phrasings for the SAME step still all resolve through that one `parse` function, so they can never drift from each other (proves AC-4/AC-4b/AC-5).
- BR-4b (generalised, replaces a v1 `step.id==='guests'` special case) A step reports "understood but rejects the value" through its OWN `parse` return (`BookingStepParseResult`'s `{ok:false, kind:'rejected', reasonKey, data?}`), never through a check keyed on which step is current. `advanceBookingFlow`/`applyStepParseResult` read only the discriminated result, never a step id — a future step that must reject an answer (an out-of-stock `gear` step) gets identical, correct treatment for free (proves AC-6, and the generic-mechanism test using a throwaway rejecting result with no step wired into the registry).
...
- EC-2b IF a chip's own slot value is submitted (`kind:'chip'`) THEN it is merged directly, bypassing `parse`/`resolveDatesCore` entirely — never dependent on that step's parse vocabulary covering the chip's value shape (BR-4).
- EC-3 IF the guests-step text names `0`, a negative number, a non-integer decimal (`"-5 คน"`, `"3.5 คน"`), or cannot be read as a number (Arabic digit or a Thai number word 0-99) at all THEN it reprompts as `unparsed`, never advances with a value the camper did not literally say (BR-4).
...
- AC-4b (chip bypass) → unit, same file ("chip input bypasses parse entirely"), including the regression case proving the SAME value submitted as text does not parse.
