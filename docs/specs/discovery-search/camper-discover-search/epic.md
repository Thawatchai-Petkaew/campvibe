...
- Out:
  ...
  - Stored `CampSite.avgRating` column (scale guard when catalog > ~200 campsites) → C-2.5 (display on card delivered via CAM-147; stored column is the scale-guard follow-up).
...
| [CAM-147](CAM-147-แสดงคะแนนรีวิวจริงบนการ์ดและ-map-popup-เลิก-hardco/story.md) | แสดงคะแนนรีวิวจริงบนการ์ด + map popup เลิก hardcode 4.8 (C-2.5) | ux-designer → frontend → qa → security → devops | **Done** (staging) |
...
- v4 (2026-06-23) — CAM-147 แสดงคะแนนรีวิวจริงบนการ์ด + map popup (C-2.5 display) Done: added Stories row; Out scope note updated (display delivered, stored-column scale-guard still pending)
