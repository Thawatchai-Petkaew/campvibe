...
| [ADR-013](ADR-013-ai-chat-persistence-agency-foundation.md) | AI chat foundation: persist history for authed users only (`ChatConversation`/`ChatMessage`, hard-delete + 180-day retention); real multi-role agent loop (cap 4); tiered tool registry (guest/authed, server-bound `userId`, no identity in schema) + 4 personal tools; additive `blocks[]` envelope + request union (AI Camping Assistant epic, CAM-266, S1 = CAM-413) | Proposed (G2) — spend + retention are the human tap points, see file note |
