...
**Status:** ACCEPTED — **ratified by owner in-chat 2026-07-19; all recommended options chosen** (guest-no-persist · 180-day hard-delete retention · caps 200/20 · agent loop cap 4 with a key-level credit ceiling · additive `blocks[]` envelope). The two G2 tap points — **spend** (D4's multi-round loop ~2× the worst-case per complex question, now bounded by an explicit key-level credit ceiling) and **retention** (D2's 180-day hard-delete of chat history) — are both accepted. Everything else follows the standing rules (ADR-009 no-merge tool layer, `.claude/rules/security.md` AI-layer, `.claude/rules/architecture.md` atomic data). · **Epic:** AI Camping Assistant (CAM-266) · **Story:** S1 (CAM-413) · **Date:** 2026-07-19
