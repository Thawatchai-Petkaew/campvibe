...
> **Note 2026-07-28 ([ADR-016](ADR-016-camper-direct-booking-and-in-chat-completion.md)):** the "without a host confirming" clause above still holds **for these HostOS models** (lead/quote/hold/deposit are the host's own ledger). It no longer holds for the **camper booking path**, where the camper's own explicit confirm is what authorizes the write. The half that is universal — *the model never performs the write; a deterministic code path does* — is unchanged everywhere.
...
6. **Low-stakes migration window:** because public/platform **payment** is deferred behind `BookingReadinessGate` (M7.5) and prod is still behind the Coming-Soon gate, there is **no real production platform-booking data at stake** for this change today (note 2026-07-28 / ADR-016: camper booking itself is NOT deferred — it has shipped since M0; re-check real booking volume before reusing this rationale) — Staging may hold seed/test rows only. This is a materially safer time to take this schema risk than after real guest bookings exist.
