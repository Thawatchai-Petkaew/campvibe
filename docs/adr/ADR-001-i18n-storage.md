...
**Status:** Superseded (2026-07-04) — the de-facto paired-column pattern (`nameTh`/`nameEn` on each localized entity) is adopted as the standing decision; the Translation table was never built and current scale (single locale pair, TH-first) does not warrant the join cost. Revisit only if a third locale is added. Evidence: prisma/schema.prisma carries paired columns throughout; CAM-102 (import-era Done) shipped only the multi-region half.
